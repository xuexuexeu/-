...
@property (nonatomic,strong)NSMutableArray *laneWaitTimeArr;  //存储每条弹道上面  还需要多久时间会空出来
@property (nonatomic,strong)NSMutableArray *laneLeftTimeArr;  //
@property (nonatomic,strong)NSMutableArray *danmakuViewArr;  //存储弹幕lab对象
...
- (void)tapClick:(UITapGestureRecognizer *)tap {
    CGPoint point = [tap locationInView:tap.view];  //获取点击坐标
...
- (void)checkAndBiu {
    ...
    // 实时更新弹道记录的时间信息  因为用了  定时器来检查  所以每条弹道的等待时间就都需要减去0.1s
...
- (BOOL)checkBoomAndBiuWith:(id<FDanmakuModelProtocol>)model {
    ...
    for (int i = 0 ; i < kDandaoCount; i++) {
        ...
        //重置数据
        // 距离/秒 = 速度v   1.当前这个弹道上开始移动的这个lab需要多久时间移动完成
